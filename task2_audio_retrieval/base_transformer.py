...
Original file is located at
    https://colab.research.google.com/drive/16QnxYZ5Sr2OuTkrgypBP0IJCNCXck-25
"""

from google.colab import drive
drive.mount('/content/gdrive', force_remount=True)

!pip install -r gdrive/MyDrive/dcase2022/requirements.txt

cd gdrive/MyDrive/dcase2022/

training_config = {
    "train_data": {
        "input_path": "/content/gdrive/MyDrive/dcase2022/Clotho.v2.1",
        "dataset": "Clotho.v2.1",
        "data_splits": {
...
def generate_sentence_embeddings(model, json_path):
  """Read the json file, dump the sentence embeddings 
     generated from the model and save the corresponding
     embedding to a new json file (with extra column 
     named <sentence_embedding> """
  model.eval()
  json_df = pd.read_json(json_path)
  captions = list(json_df.original)
  embeddings = model.encode(captions)
  embeddings = [embeddings[i,:] for i in range(embeddings.shape[0])]
  json_df["sentence_embedding"] = embeddings
  out_path = json_path[:-5]+".pkl"
  print("Saving to {}".format(out_path))
  json_df.to_pickle(out_path)

filenames = ["development_captions.json",
             "evaluation_captions.json",
             "validation_captions.json"]
data_dir ="Clotho.v2.1/"
for f in filenames:
  generate_sentence_embeddings(sentence_model, data_dir+f)
...
model.load_state_dict(torch.load("train/baseline/models/"+"model_baseline_gru_st_20.pth"))
...
for i in range(1, 41):
    print(i)
    model.load_state_dict(torch.load("train/baseline/models/"+"model_baseline_gru_st_{}.pth".format(i)))
    ...
    def retrieval_metrics(gt_csv, pred_csv):
        ...
        r10.append(metrics["R10"])
        map.append(metrics["mAP10"])
    ...
    pred_csv = "output_{}.csv".format(i)  # baseline system retrieved output for Clotho evaluation data

    retrieval_metrics(gt_csv, pred_csv)

x = np.array([r1,r5,r10,map])

np.save(open("transformer_metrics.npy",'wb'), x)

print(np.load(open("transformer_metrics.npy",'rb')))
