...
from utils import data_utils_sep
text_datasets, vocabulary = data_utils_sep.load_data(training_config["train_data"])
...
for split in ["train", "val", "test"]:
    ...
    _loader = DataLoader(dataset=_dataset, batch_size=BATCH_SIZE,
                          shuffle=True, collate_fn=data_utils_sep.collate_fn)
...
model2 = torch.load(model_dir + 'model3_13.pth', map_location=device)
model.load_state_dict(model2)
"""
outfile = open(info_dir + "logs3.txt", 'w')
...
for epoch in range(100):
  ...
  torch.save(model.state_dict(), model_dir + "model3_" + str(epoch+1) + ".pth")
...
"""
#for split in ["test"]:
#    eval(model, criterion, text_loaders[split], split, None)
...
for split in ["test"]:
    output = audio_retrieval(test_model, text_datasets[split], K=10)
