...
class QueryAudioDataset(Dataset):
    ...
    def __init__(self, audio_feature, data_df, query_col, vocabulary):
        ...
        self.vgg = torch.load('../saved_data/vgg.pt')

    def get_vgg(self, fid):
        if fid in self.vgg['development']:
            return self.vgg['development'][fid]
        if fid in self.vgg['validation']:
            return self.vgg['validation'][fid]
        if fid in self.vgg['evaluation']:
            return self.vgg['evaluation'][fid]
        return None
    ...
    def __getitem__(self, index):
        ...
        #audio_feat = (audio_feat - audio_feat.mean(axis=0))/audio_feat.std(axis=0) # Cepstral Normalization
        ...
        audio_feat = torch.from_numpy(self.get_vgg(item["fid"]))
...
def collate_fn(data_batch):
    ...
    #audio_feat_batch = (audio_feat_batch - audio_feat_batch.mean(axis=0))/audio_feat_batch.std(axis=0) # Cepstral Normalization
