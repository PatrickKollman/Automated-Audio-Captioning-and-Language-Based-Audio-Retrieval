...
class CRNNEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        rnn_in_dim = 384
        self.gru = nn.GRU(rnn_in_dim, rnn_in_dim, bidirectional=True, batch_first=True)
...
class BARTAAC(nn.Module):
    def __init__(self, settings, device):
        ...
        if n_adapt_layers >= 1:
            audio_adapt_list = [nn.Linear(audio_emb_size, lm_emb_size//2),
                                nn.Linear(lm_emb_size//2, lm_emb_size)]
    ...
    def forward(self,
        ...
        # # CRNN encoder
        # embeds = self.crnn(audio_features)
        # encoder_outputs = [embeds]

        # Average Hidden State Encoder
        ...
        #             return_dict=True)
        # hiddens = torch.stack(encoder_outputs['hidden_states'])
        # avg = torch.mean(hiddens, dim=0)
        # encoder_outputs = [avg]

        # M-Hidden States Encoder
        encoder_outputs = self.bart_lm.model.encoder(
                    input_ids=None,
                    attention_mask=attention_mask,
                    head_mask=head_mask,
                    ...
                    output_hidden_states=True,
                    return_dict=True)
        m_states = (encoder_outputs['hidden_states'][-1], encoder_outputs['hidden_states'][-2]) 
        hiddens = torch.stack(m_states)
        avg = torch.mean(hiddens, dim=0)
        encoder_outputs = [avg]
    ...
    def generate_beam(self,
        ...
        # # CRNN encoder
        # embeds = self.crnn(audio_features)
        # encoder_outputs['last_hidden_state'] = embeds

        # Average Hidden State Encoder  
        # hiddens = torch.stack(encoder_outputs['hidden_states'])
        # avg = torch.mean(hiddens, dim=0)
        # encoder_outputs['last_hidden_state'] = avg

        # M Hidden State Encoder
        m_states = (encoder_outputs['hidden_states'][-1], encoder_outputs['hidden_states'][-2]) 
        hiddens = torch.stack(m_states)
        avg = torch.mean(hiddens, dim=0)
        encoder_outputs['last_hidden_state'] = avg
        
        # Last Hidden State Implementation
