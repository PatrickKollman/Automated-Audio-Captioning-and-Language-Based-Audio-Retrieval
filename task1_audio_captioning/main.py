...
def main(config):
    ...
    if settings['workflow']['train']:
        data_train, data_eval = get_dataset('development', settings, tokenizer)
        print('Loaded development dataset.')
    
    # Model
    model = BARTAAC(settings, device)
    if settings['load']['checkpoint']:
        model.load_state_dict(torch.load(settings['load']['checkpoint'], map_location=device))
